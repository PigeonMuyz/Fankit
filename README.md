...
```sh
./script/build_release_dmg.sh 1.0.9
```

The output is written to `dist/Fankit-1.0.9.dmg` together with its SHA-256
...
Release. Users who need a higher API rate limit can enter their own GitHub API
token in Updates settings. The optional token is stored in Fankit's regular
preferences and is only added to GitHub API requests; it does not need repository
write permission.

Release DMGs should keep the `Fankit-<version>.dmg` name and include either
